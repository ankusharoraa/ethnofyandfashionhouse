-- Fix ledger/accounting correctness: when customer advance is applied on a sale,
-- it must be reflected in customer_ledger so ledger balance stays in sync with (outstanding - advance).
