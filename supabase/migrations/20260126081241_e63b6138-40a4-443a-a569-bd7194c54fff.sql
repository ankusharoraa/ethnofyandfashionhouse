-- Fix customer ledger: use total_amount for sale debit instead of (pending + advance_applied)
-- This ensures fully-paid bills still show in the ledger
...
BEGIN
  ...
  IF p_customer_id IS NOT NULL THEN
    ...
    -- FIXED: Ledger sale entry now uses total_amount as debit (not just pending + advance_applied)
    -- This ensures fully-paid bills appear in ledger with correct debit
    IF v_total_amount > 0 THEN
      PERFORM public.append_customer_ledger(
        ...
        p_invoice_id,
        v_invoice_number,
        v_total_amount,  -- Changed from (v_pending + v_advance_applied)
        0,
        now()
      );
    END IF;
    ...
    -- Ledger: remaining overpayment to advance
    IF v_to_advance > 0 THEN
      INSERT INTO public.customer_payments (
        ...
        v_to_advance,
        p_payment_method,
        'Overpayment to advance on ' || v_invoice_number,
        auth.uid(),
        p_invoice_id
      ) RETURNING id INTO v_overpay_payment_id;
